...
const Produs& Service::cautaProdus(const std::string& numeProdus) const {
...
std::unordered_map<string, int> Service::raportSrv() {
    std::unordered_map<string, int> map;
    const vector<Produs>& produse = repository.getAll();
    for (const auto& produs : produse) {
        if (map.find(produs.getTipProdus()) != map.end())
            map[produs.getTipProdus()]++;
        else
            map[produs.getTipProdus()] = 1;
    }
    return map;
}
...
vector<Produs> Service::sorteaza(int reperSortare, int ordineSortare) {
    ...
    vector<Produs> produse = repository.getAll();

    bool(*functieDeComparare)(const Produs & produs1, const Produs & produs2) { comparareDupaProducatorCrescator };
    ...
    sort(produse.begin(), produse.end(), functieDeComparare);
    return produse;
}
